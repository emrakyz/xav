...
#[unsafe(no_mangle)]
pub extern "C" fn xav_x265_log(msg: *const c_char, len: c_int) {
...
pub fn x265_crf(p: *mut u8, crf: f32) {
    unsafe { p.add(OFF_RF_CONSTANT).cast::<f64>().write(f64::from(crf)) };
}
...
pub fn x265_parse(dst: *mut u8, preset: &[u8], tune: &[u8], args: &[u8], zone: &[u8]) {
    ...
    parse_pairs(dst, [args, zone], x265_param_parse, "x265");
...
#[cold]
#[inline(never)]
pub fn x265_simd(tmpl: &[u64]) {
    let mut p = MaybeUninit::<ParamBuf>::uninit();
    let d = p.as_mut_ptr().cast::<u8>();
    unsafe {
        copy_nonoverlapping(tmpl.as_ptr().cast::<u8>(), d, X265_PARAM_SIZE);
        xav_x265_setup(d);
    }
}

#[inline]
pub fn x265_open(d: *mut u8, frames: usize) -> *mut c_void {
